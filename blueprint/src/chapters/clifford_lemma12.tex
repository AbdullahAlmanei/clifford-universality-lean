...
\subsection{Distance-independent rotation approximation}

\begin{theorem}[$R_z$ approximation by $\{H,T\}$]
  \label{thm:lemma12_projective}
  \lean{TwoControl.Clifford.Lemma12.G1G2.HT_rz_dense_projective}
  \leanok
  Let $d$ satisfy the projective distance interface.  For every $\theta$ and
  every $\epsilon>0$, there is an $\{H,T\}$ word $C$ such that
  \[
    d\bigl(R_z(\theta),\operatorname{eval}(C)\bigr)<\epsilon.
  \]
\end{theorem}

\begin{proof}
  \leanok
  \uses{lem:g_axes, lem:euler_orthogonal, lem:rz_axis,
        def:projective_distance}
  Use positive powers of the two irrational axis rotations to approximate the
  three factors in the exact orthogonal-axis Euler decomposition.  Continuity
  gives each one-axis approximation.  Global-phase consistency removes the
  scalar phases in the circuit words.  Multiplicative subadditivity bounds the
  error of the product by the sum of the three errors.
\end{proof}
