...
`distance-of-conjugated-unitaries` (Lemmas 9–11 of the synthesis development) -/
...
/-! ### Hilbert--Schmidt distance as a projective distance measure -/

theorem hsDistance_nonnegative {N : ℕ} (U V : Square N) :
    0 ≤ hsDistance U V :=
  Real.sqrt_nonneg _
...
theorem hsDistance_consistency {N : ℕ}
  ...
  unfold hsDistance
  have htrace :
      Matrix.trace ((z • U₂)† * (w • V₂)) =
        (star z * w) * Matrix.trace (U₂† * V₂) := by
    rw [Matrix.conjTranspose_smul, Matrix.smul_mul, Matrix.mul_smul,
      smul_smul, Matrix.trace_smul, smul_eq_mul]
  have hphase : ‖star z * w‖ = 1 := by
    rw [norm_mul, norm_star, hz, hw, one_mul]
  rw [htrace, norm_mul, hphase, one_mul]

open scoped Matrix.Norms.Operator in
theorem hermitianEvolution_mem_unitaryGroup {N : ℕ}
    (A : Square N) (hA : A.IsHermitian) (t : ℝ) :
    hermitianEvolution A t ∈ Matrix.unitaryGroup (Fin N) ℂ := by
  have hself : IsSelfAdjoint A := by
    simpa [Matrix.star_eq_conjTranspose] using hA
  have hscalar : (Complex.I * (t * Real.pi) : ℂ) ∈ skewAdjoint ℂ := by
    rw [skewAdjoint.mem_iff]
    simp
  have hskew : ((Complex.I * (t * Real.pi) : ℂ) • A) ∈
      skewAdjoint (Square N) := hself.smul_mem_skewAdjoint hscalar
  have hunit := NormedSpace.exp_mem_unitary_of_mem_skewAdjoint hskew
  rw [Matrix.mem_unitaryGroup_iff']
  exact (Unitary.mem_iff.mp hunit).1

theorem continuous_hsDistance_right {N : ℕ} (U : Square N) :
    Continuous (fun V : Square N => hsDistance U V) := by
  unfold hsDistance
  have hmul : Continuous (fun V : Square N => U† * V) :=
    continuous_const.matrix_mul continuous_id
  have htrace : Continuous (fun V : Square N => Matrix.trace (U† * V)) := by
    unfold Matrix.trace
    exact continuous_finset_sum Finset.univ (fun i _hi => hmul.matrix_elem i i)
  exact (continuous_const.sub
    ((htrace.norm.pow 2).div_const ((N : ℝ) ^ 2))).sqrt

open scoped Matrix.Norms.Operator in
theorem continuous_hermitianEvolution {N : ℕ} (A : Square N) :
    Continuous (hermitianEvolution A) := by
  unfold hermitianEvolution
  apply NormedSpace.exp_continuous.comp
  fun_prop

open scoped Matrix.Norms.Operator in
theorem hsDistance_hermitianEvolution_continuity {N : ℕ} (hN : 0 < N)
    (A : Square N) (hA : A.IsHermitian) (t₀ ε : ℝ) (hε : 0 < ε) :
    ∃ δ, 0 < δ ∧ ∀ t, |t₀ - t| < δ →
      hsDistance (hermitianEvolution A t₀) (hermitianEvolution A t) < ε := by
  let f : ℝ → ℝ := fun t =>
    hsDistance (hermitianEvolution A t₀) (hermitianEvolution A t)
  have hf : Continuous f :=
    (continuous_hsDistance_right (hermitianEvolution A t₀)).comp
      (continuous_hermitianEvolution A)
  obtain ⟨δ, hδ, hbound⟩ := Metric.continuousAt_iff.mp hf.continuousAt ε hε
  refine ⟨δ, hδ, ?_⟩
  intro t ht
  ...
  have h := hbound hdist
  have hself : f t₀ = 0 :=
    hsDistance_self hN _ (hermitianEvolution_mem_unitaryGroup A hA t₀)
  rw [Real.dist_eq, hself, sub_zero,
    abs_of_nonneg (hsDistance_nonnegative _ _)] at h
  exact h

/-- Hilbert--Schmidt distance satisfies the four abstract projective-distance
laws used by the distance-independent universality theorem. -/
noncomputable def hsProjectiveDistanceMeasure {N : ℕ} (hN : 0 < N) :
    ProjectiveDistanceMeasure N where
  distance := hsDistance
